...
% roughness : 3
% color : 8
sv = zeros(1,2,2,3,3,8);
selectionScale = zeros(2,3,2,3,3,8);
BSsample = zeros(B,stimnum,2,3,3,8);
...
for i = 1:2 % light
    for j = 1:3 % diffuse
        for k = 1:3 % roughness
            for l = 1:8 % color
                fprintf('light:%d, diffuse:%d, roughness:%d, color:%d \n\n', i,j,k,l);
                ...
                estimated_sv = TNT_FCN_PCanalysis_Thurston(mtx(:,:,i,j,k,l), 0.005);
                ...
                [estimated_sv2,NumGreater_v,OutOfNum_v] = TNT_FCN_PCanalysis_ML(OutOfNum(:,:,i,j,k,l), NumGreater(:,:,i,j,k,l), cmbs, InitValues, method);
                ...
                ranges95_th(s,3) = estimated_sv(s);

                % for ML data
                sdata = sort(sv_ml(:,s));
                ranges95_ml(s,1) = sdata(lbi)-estimated_sv2(s); % lower bound
                ranges95_ml(s,2) = sdata(ubi)-estimated_sv2(s); % upper bound
                ranges95_ml(s,3) = estimated_sv2(s);
                end

                % record data
                sv(:,:,i,j,k,l) = estimated_sv2;
                selectionScale(:,:,i,j,k,l) = ranges95_ml;
                BSsample(:,:,i,j,k,l) = sv_ml; % bootstrap sample
