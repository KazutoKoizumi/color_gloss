...
% color : 8
sv = zeros(1,2,2,3,3);
selectionScale = zeros(2,3,2,3,3);
...
tnum = 8; % trial number in each stimulus pair in conventional experiment
...
for i = 1:2 % light
    for j = 1:3 % diffuse
        for k = 1:3 % roughness
            fprintf('light:%d, diffuse:%d, roughness:%d \n\n', i,j,k);
            ...
            estimated_sv = TNT_FCN_PCanalysis_Thurston(mtx(:,:,i,j,k), 0.005);
            ...
            [estimated_sv2,NumGreater_v,OutOfNum_v] = TNT_FCN_PCanalysis_ML(OutOfNum(:,:,i,j,k), NumGreater(:,:,i,j,k), cmbs, InitValues, method);
            ...
            % record data
            sv(:,:,i,j,k) = estimated_sv2;
            selectionScale(:,:,i,j,k) = ranges68_ml;
