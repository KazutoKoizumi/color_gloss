...
try
    ...
    load('../../stimuli/patch/patch_high_luminance.mat');
    load('../../stimuli/patch/patch_high_luminance_gray.mat');
    load('../../stimuli/patch/rgb_patch_high_luminance_gray.mat');
    ...
    %trashTrialNum = 6;
    trashTrialTime = duration(0,0,30); 
    ...
    varTypes = {'uint8','uint8','string','uint8','uint8','uint8'};
    varNames = {'luminance','saturation','color','grayRGB1','grayRGB2','grayRGB3'};
    if sessionNum == 1  
        % make data table
        dataTable = table('Size',[stimuliN,6],'VariableTypes',varTypes,'VariableNames',varNames);
        
        % 各セッションの呈示順
        order = zeros(stimuliN,3);
        for i = 1:3
    ...
    while i <= sessionTrialNum
        ...
        if oneOrTwo == 1 % 有色パッチが左
            rgbLeft = patch_high_luminance(:,:,:,index(stiNum,3),index(stiNum,1),index(stiNum,2));
            rgbRight = patch_high_luminance_gray;
            % 無色パッチの位置
            posGray = [rightPosition(1)+px,rightPosition(2)+py,rightPosition(1)+px_max,rightPosition(2)+py_max];
        else % 有色パッチが右
            rgbLeft = patch_high_luminance_gray;
            rgbRight = patch_high_luminance(:,:,:,index(stiNum,3),index(stiNum,1),index(stiNum,2));
            % 無色パッチの位置
        ...
        rgbGray = rgb_patch_high_luminance_gray;
        grayVal = round(mean(rgbGray));
